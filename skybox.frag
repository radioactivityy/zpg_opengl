...
void main()
{
    ...
    // Only use horizontal angle (yaw), keep vertical as-is for natural look
    float yaw = atan(ray_dir.y, ray_dir.x);  // -PI to PI

    // Map yaw to U coordinate (180 degrees = full image width)
    // Center the image and spread across 180 degrees
    float u = (yaw / PI) * 0.5 + 0.5;  // 0 to 1 for 180 degrees

    // For V, use a gentle curve based on pitch to avoid distortion
    // This gives a natural horizon look
